...
describe('enrich-events integration', () => {
  it('stamps only applicable Stage-2 enrichment keys per source/event_type and remains idempotent', () => {
    const events = [
      {
        id: 'evt-1',
      ...
      {
        id: 'evt-3',
        timestamp: '2026-04-13T14:00:00.000Z', // Monday
        source: 'zeek',
        event_type: 'rdp',
        src_ip: '203.0.113.10',
        dest_ip: '10.20.30.40',
    ...
    assert.deepEqual(
      Object.keys(once[2].enrichment).sort((left, right) => left.localeCompare(right)),
      ['business_hours'],
    );
    assert.equal(once[0].enrichment.business_hours, true);
    assert.equal(once[1].enrichment.business_hours, false);
    assert.equal(once[2].enrichment.business_hours, true);
